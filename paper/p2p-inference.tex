...
\title{Quantifying Carbon, Water, Cost, and Capability in\\
Peer-to-Peer Inference}

\author{\IEEEauthorblockN{AutoYou Research}
\IEEEauthorblockA{%
\textit{Decentralized Inference \& Sustainable Compute Group}\\
...
approximately 945\,TWh by 2030, about the consumption of Japan, with AI a major
driver of the growth. A widely proposed remedy is to decentralize it: run small
...
each receive an explicitly scoped supported, partial, or refuted verdict.
...
threshold, above the 40--70\% agentic-replaceability estimates in NVIDIA's case
studies; our workload mix is an authorial assumption rather than a sampled
corpus - edge execution cuts per-query carbon by $54$--$64\%$ against typical and
heavy frontier usage with positive savings in all model draws, nearly
eliminates the modeled data-centre on-site cooling-water term, and costs
approximately 150 times less per
...
RTX 5070 discrete GPU (12\,GB GDDR7) - grounds edge decode and prefill in direct
observation, with board-energy observation on the NVIDIA platform. On the APU,
single-stream generation reaches
...
(3B) and \RtxEightEnergy{}\Wh\ (8B) at 499 prompt and 300 output tokens, repeated
...
100\,W. The models, datasets, telemetry harnesses, and an empirical routing
agent are included in the reproducibility package, and every measured row is
reported rather than filtered. The two hardware targets were operated by the
authors: no independent operator or physical-target replication is included,
and the AMD concurrent matrix is timing-only because no usable power sensor
was available.
\end{abstract}
...
infrastructure-aware benchmark estimates a short GPT-4o query at $0.42\Wh$ and
a single long reasoning query (o3, DeepSeek-R1) above $30\Wh$ \cite{jegham2025}.
Yet
data-centre power-usage effectiveness (PUE) is reported at a global average of
...
GPT-4o and Claude-3.5 on tool calling. Their case studies estimate that
$40$--$70\%$ of LLM calls in three agentic systems (MetaGPT, Open Operator,
Cradle) are SLM-replaceable; we treat this as author-reported evidence, not as
an independent population estimate.
...
read against NVIDIA's case-study agentic-replaceability estimates, which it exceeds
(\S\ref{sec:results}).
...
model-scoped verdict backed by a 50k-draw Monte-Carlo and a sensitivity tornado, including an
...
measured ones, including board energy on the discrete GPU, and puts the
bandwidth-bound premise behind the device-class argument to a test it could
fail --- and partly did.
...
PUE captures facility overhead (reported global $1.54$ \cite{uptime2025};
hyperscale $1.09$--$1.15$ in the cited production and survey contexts). The EU
Energy Efficiency Directive now mandates data-centre
...
single figure fits a global average. Reported grid carbon intensity averages
$458\gco$/kWh
globally, spanning reported EU $\sim$140 to India $\sim$708 \cite{ember2026}. Jegham
\emph{et al.}\ \cite{jegham2025} estimate per-query energy across model sizes;
...
idle/redundancy/overhead $18\%$). That is a margin excluded from Google's
cloud boundary, not a guaranteed saving on an edge device whose host, idle,
and power-supply draw must be counted separately.
...
\textbf{The current frontier and price sensitivity.}
Commercial model rosters and prices changed between the measurement corpus and
this analysis. We use dated provider pricing pages only as economic inputs;
they are not evidence for release chronology, model architecture, or per-query
energy \cite{frontier2026,anthropic_pricing2026,google_pricing2026}. The energy,
water, and carbon results therefore remain pinned to the measured models. In the
source set used for this release, we found no public per-query energy measurement
for the currently priced models; this is a search result, not proof that no such
measurement exists. The decision rule consequently treats the displaced cloud
baseline as a parameter rather than silently transferring an older model's
energy to a newer one. That keeps the efficient-cloud counter-case of H5 visible.

\textbf{Edge inference and P2P transport.}
Edge execution may cut energy for suitable tasks
\cite{techtarget2025} --- a trade-press directional claim that we do not use
as a quantitative input --- but consumer accelerators can be less efficient
...
ratio, in C2. We use the STUN/TURN split in Table~\ref{tab:params} only as a
scenario informed by a vendor note, not as a measured deployment fraction
\cite{antmedia2026}; the
...
Three bodies of work quantify cloud inference energy, and the gap between them
is what this paper's Pass 4 addresses. Jegham \emph{et al.} \cite{jegham2025}
estimate per-query energy, water and carbon across model sizes using public API
data, inferred hardware, and batch assumptions, including a batch-8 scenario on
H100/H200-class server accelerators. Elsworth \emph{et al.}
\cite{elsworth2025} report a comprehensive at-scale measurement of a production
TPU fleet and are the only source in our set that closes the loop from
accelerator to facility. Caravaca \emph{et al.} \cite{caravaca2025} contribute
over 32{,}500 direct GPU inference-energy measurements and a predictive
per-query model, again on data-centre parts. The first is model-based; the latter
two are measurement studies. All three concern the \emph{cloud} side.
...
$15$--$25\%$ \cite{luccioni2025}. We model embodied hardware carbon as a
$15$--$30\%$ sensitivity range, informed by life-cycle accounting work but not
directly established by the retrieved source \cite{schneider2025}. We charge
both against our savings.
...
with independent triangular distributions over each analyst-declared parameter
range and report mean, percentiles, and $P(S>0)$ per cloud baseline, plus a
one-at-a-time tornado. Here $P(S>0)$ is the fraction of declared model draws
with positive savings, not a confidence level or posterior probability about
the real world.
...
\midrule
PUE (hyperscale / global) & 1.10 / 1.54 & reported contexts; \cite{uptime2025,elsworth2025} \\
WUE on-site (hyperscale) & 0.20 [0.12, 0.30] L/kWh & model; informed by \cite{aws_water2025} \\
EWIF (grid water) & 1.10 [0.50, 2.50] L/kWh & assumption \\
Grid CI (global) & 458 [400, 473] g/kWh & reported input; \cite{ember2026} \\
$E$ Gemini / GPT-4o / o3 & 0.24 / 1.79 / 39.2 Wh & \cite{elsworth2025,jegham2025} \\
$E$ Llama-3.1 8B (cloud) & 0.60 Wh & \cite{jegham2025} \\
RTX 4090 (8B) throughput & 141 tok/s & legacy scenario; \cite{databasemart2026} \\
...
Desktop idle power & 60 [30,150] W & sensitivity assumption; \cite{solartech2025} \\
STUN direct / TURN relay & 0.80 / 0.15 & scenario; \cite{antmedia2026} \\
Net intensity & 0.015 [0.006,0.06] kWh/GB & sensitivity; informed by \cite{aslan2018} \\
Rebound factor & 0.20 [0.15,0.25] & \cite{luccioni2025} \\
Cloud out/in price & \$15 / \$3 per Mtok & dated price snapshots; \cite{frontier2026,anthropic_pricing2026,google_pricing2026} \\
...
\bottomrule
\end{tabular}
\footnotesize The Gemini value is Google's measured median; the GPT-4o,
o3, and Llama-3.1 values are Table 4 values from Jegham v2's model-based
estimate, not measurements from this study.
\end{table}
...
\midrule
H1 & P2P lowers network transmission energy & \checkmark & \textcolor{edgedark}{\textbf{Supported (model)}} \\
H2 & Right-sizing cuts inference energy 3--65$\times$ & \checkmark & \textcolor{edgedark}{\textbf{Supported (bounded)}} \\
...
H6 & Relay plaintext access prevented (DTLS) & \checkmark & \textcolor{edgedark}{\textbf{Supported (protocol)}} \\
\midrule
C1 & Rebound cancels savings & $\sim$ & \textcolor{cloudorange}{\textbf{Modelled (conditional)}} \\
...
\subsection{H2: Right-sizing dominates (Supported within model)}
...
hard}$ has an arithmetic basis: $0.40/0.78=0.51$ from reported hard-math
...
NVIDIA's case studies estimate $40$--$70\%$ of calls in three real agentic
systems as SLM-replaceable \cite{belcak2025}. Our $82\%$ is \emph{above that range}, not at
...
reader who prefers NVIDIA's reported range should read their own answer off
Fig.~\ref{fig:tornado}, where $\fs$ is now swept across $[0.40, 0.90]$ as a
first-class factor: at the midpoint of NVIDIA's reported band ($\fs=55\%$) carbon savings
...
For a capability-sufficient query, the model-based cloud frontier (GPT-4o)
estimate costs
...
whichever is larger. Within these sampled model assumptions, the Monte-Carlo
(Fig.~\ref{fig:savings}, Table~\ref{tab:mc}) returns positive savings against
typical and heavy frontier baselines ($P(S>0)=1.00$). \textbf{The counter-case}: against
...
Positive only against frontier usage; refuted against best-in-class efficient cloud
(Gemini, red).}
\label{fig:savings}
\end{figure}
...
We re-price this comparison with dated first-party provider price snapshots
\cite{frontier2026,anthropic_pricing2026,google_pricing2026}. These pages are
used only as price inputs, not as evidence about release chronology, model
architecture, or energy. Across the low, middle, and premium price inputs, the
calculated edge-to-cloud ratio moves materially while preserving the same
ordering: decisive against expensive frontier tiers and materially narrower
against efficient ones. The exact ratio is therefore a scenario output, not a
...
one here with no source; its low end is the floor of NVIDIA's \emph{reported}
...
We conduct two experimental passes (a primary run and a same-target
repeatability run) comprising seven repetitions per model at a standard query
...
yielding \RtxThreeMilliWh{}\,mWh per output token. The repeatability run agrees
...
\RtxEightMilliWh{}\,mWh per output token. The repeatability run again measures
...
six rows --- two models, two query shapes, and a same-target repeatability run
for each. $\beta$ is evaluated from the fitted prefill line at each row's own prompt
...
\end{tabular}
\end{table}
\subsection{H1 \& H6: Transport (Supported within protocol)}
...
simplification arithmetically and now supplies a public synthetic benchmark
for the narrower question of held-out task accuracy. Its three model clusters,
five classes, 20 evaluation tasks per class and three adapter seeds are useful
evidence, but they do not sample the workload behind $\fs$ or provide an
independent device replication.

\emph{The crux variable is conditionally invariant at the default bar.} At this
paper's acceptance threshold $\alpha=0.70$, the declared adaptation scenario changes $\fs$ by
...
adaptation}. What improves instead is measured correctness on the companion's
synthetic held-out tasks: $\AdaptationBaseAccuracy\%$ to
$\AdaptationAdaptedAccuracy\%$, or $+\AdaptationUplift\pp$. That result is
not a measurement of the workload the local model was already serving. The
workload-weighted $88.4\%$ to $93.9\%$ figure remains an exploratory scenario,
and the companion study is explicit about the distinction.
...
the companion study's memory model reproduces a publicly documented 27B run to within
...
\textbf{C1: Rebound (Modelled, conditional).} One rebound model, reported in two
...
$212\%$. Both figures are reported in both units wherever they appear. Under
the assumed $15$--$25\%$ take-back range, rebound erodes but does not erase the
modelled savings; real-world deployment rebound is not measured here and remains
unresolved.
...
execution structurally cannot. Our AMD timing-only batch matrix provides a
within-target check of this distinction: aggregate throughput at batch sizes
$1$, $2$, $4$ and $8$ was compared with paired repetition IDs, but the host had
no usable power sensor. A separate 10-repetition matrix for Qwen3.8 27B on the
same target measured median aggregate throughput of $18.1$, $18.7$, $17.8$ and
$17.0$ tok/s at those batch sizes: a small batch-2 gain followed by lower
throughput at batch 4 and 8. This is still a single-target timing result, not
an energy measurement or a production-serving law. \emph{Edge therefore does not win on per-token
efficiency}, and running a \emph{large} model at the edge is a net loss. We
decline to quote an energy ratio: the batch trace has no power measurement, and
the per-token figures circulating for a data-centre deployment are not
comparable to a board-rail measurement without a stated accounting boundary.
The edge advantage claimed in this paper is right-sizing, water elimination,
overhead removal and marginal energy --- none of which is a per-token efficiency
claim.
...
compare $\fs$ with NVIDIA's case-study replaceability estimates \cite{belcak2025}. Cloud
per-query energy depends on prompt length/batch; we use reported short/long
...
current priced frontier models have no per-query energy measurement in the
source set used for this release; that is a search result, not proof of absence.
Their serving energy may fall anywhere between our frontier and efficient-cloud
baselines. The decision rule spans that interval by construction, since it
selects on the displaced baseline rather than assuming either extreme, but a
measurement of current models remains the most valuable single addition to this
analysis.

\textbf{Scope of our own measurement (\S\ref{sec:measured}).} Two machines: an
AMD unified-memory APU and an NVIDIA discrete GPU. The primary energy records
are single-stream; the AMD target also has a concurrent timing matrix at batch
sizes $1$, $2$, $4$ and $8$, with no usable power sensor. While NVIDIA telemetry
provides direct board-level power sampling and bracketed request integration,
board power is not whole-system wall power: host
...
(\S\ref{sec:measured}). The timing matrix is not a production-serving or
energy-efficiency result: it does not measure wall power, sustained thermal
behavior, queueing, or a matched cloud batch. The bandwidth ladder rests on
peak figures taken from hardware specifications rather than measured transfer
ceilings, and
...
the inherited model raises no coverage at ordinary thresholds, while the
synthetic benchmark shows a task-accuracy gain, a system should adapt to
improve the answers it already gives locally and should not treat an adapter as
a reason to widen what it attempts.
...
are capability sufficiency ($\fs\approx82\%$, compared with NVIDIA's reported $40$--$70\%$),
local grid intensity, hardware reuse, and rebound. Against typical and heavy
frontier usage the system saves $54$--$64\%$ carbon in every declared model draw
and costs $\sim$$150\times$ less per query; against a best-in-class efficient cloud model it
does not. A companion study \cite{autoyou2026adaptation} derives a conditional
coverage result and measures a $+\AdaptationUplift\pp$ gain on its synthetic
held-out task benchmark. That accuracy result is limited to three model
clusters on one physical host and does not identify the real-workload effect,
so these bounds remain conditional. A Pass-4 measurement
campaign turns the device-class argument into a within-machine test carrying its
own falsification control; the premise survives, and board power on a discrete
accelerator is directly measured and bracketed rather than assumed, while
host-level system boundaries remain explicitly parameterized. The models,
simulation harnesses and trace records behind these results are retained in full
as a reusable, falsifiable basis for sustainable-AI claims, and are released
openly.
...
This is research release v0.3.0 dated 2026-09-14. Venue, DOI and peer-review
status are intentionally unassigned in this release. Repository metadata, the
citation audit, and the planned multi-hardware and multi-model replication are
released beside this manuscript.
