...
PAPERS = ["p2p-inference.tex", "adaptation.tex"]
...
def build_vocabulary() -> Tuple[Dict[str, List[str]], int]:
    ...
    # The concurrent batch extension is generated by measure/statistics.py,
    # not by models/run_all.py. Include only its named estimates so a paper
    # number can be traced to the raw serving analysis without adding every
    # timing sample and bootstrap draw to the vocabulary.
    statistics_path = os.path.join(HERE, "..", "measure", "statistics.json")
    if os.path.isfile(statistics_path):
        with open(statistics_path, encoding="utf-8") as fh:
            statistics = json.load(fh)
        for record in statistics.get("files", []):
            if record.get("kind") != "concurrent_batches":
                continue
            filename = record.get("file", "unknown")
            for row in record.get("batch_scaling", []):
                model = row.get("model", "unknown")
                batch_size = row.get("batch_size", "unknown")
                for metric in ("aggregate_output_tok_s",
                               "paired_difference_vs_batch_1"):
                    estimate = row.get(metric, {}).get("estimate")
                    if isinstance(estimate, (int, float)):
                        source = (f"measure/statistics.json:{filename}:"
                                  f"{model}:batch_{batch_size}:{metric}")
                        add(_forms(float(estimate)), source)
...
CITED = {
    ...
    # Strix Halo recipe (publicly documented run)
    "8192": "documented 27B run: sequence length",
    "448": "documented 27B run: total steps",
    "11": "documented 27B run: minutes per step",
    "80": "documented 27B run: peak training memory, GB",
    "128": "Strix Halo unified memory, GB; also LoRA rank in the documented run",
    "256": "documented 27B run: LoRA alpha",
    ...
    "4.127": "QLoRA: bits per parameter for NF4 with double quantization "
             "(4 + 8/64 + 32/16384)",
    "32,768": "documented 27B run: tokens per optimizer step "
    ...
    "40": "NVIDIA's 40-70% agentic-replaceability lower bound",
...
def main() -> None:
    ...
    # numbers in p2p-inference.tex and nearly half in adaptation.tex match ten or more
